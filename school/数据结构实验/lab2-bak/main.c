...
void display_utf8_info(utf8 ch, huffcode code) {
    ...
    for (Byte i = 0; i < code->_size; i++) {
        printf("%d", huffcode_get(code, i) ? 1 : 0);
    }
    printf("\n");
}
...
int main() {
    ...
    // 文件路径
    const char *input_file = "test.md";
    const char *encoded_file = "test.md.huf";
    const char *decoded_file = "test.md.decoded";
    ...
    // 5. 编码文件
    printf("\n5. 编码文件...\n");
    start_time = clock();    ex = hdfile_open(hf, input_file, encoded_file);
    if (ex.status != SUCCESS) {
        printf("错误：%s\n", ex.msg);
        free_huffman(hm);
        free_sqlist(freq_table);
    ...
    // 6. 解码文件
    printf("\n6. 解码文件...\n");
    start_time = clock();    ex = hdfile_open(hf, encoded_file, decoded_file);
    if (ex.status != SUCCESS) {
        printf("错误：%s\n", ex.msg);
        free_huffman(hm);
        free_sqlist(freq_table);
